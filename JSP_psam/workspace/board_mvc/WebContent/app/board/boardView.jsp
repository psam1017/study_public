...
<script>
		...
		if(replies != null && replies.length != 0){
			$.each(replies, function(index, reply){
				...
				if("${sessionId}" == reply.memberId){
					...
					text += "<input type='button' id='ok" + (index + 1) + "' class='primary' style='display:none' onclick='updateReply(" + (index + 1) + ", " + reply.replyNum + ")' value='수정완료'>"
	...
	function comment(){
		...
		$.ajax({
			...
			// get 방식으로 보낼 때 내용이 많으면 url이 너무 길어지고 오류가 발생.
			type: "post",
	...
	function updateCancel(index){
		...
		cancel.replaceWith("<input type='button' id='remove" + index + "' class='primary' onclick='' value='삭제'>");
